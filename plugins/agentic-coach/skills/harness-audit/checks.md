...
**Fields:** `group` (guidelines · autotests · guardrails) · `tip` · `applies_when` (precondition; fails → `– n/a`, never `❌`) · `layer` (**A** = path the harness defines, absence is conclusive · **B** = the stack defines it, absence is never conclusive alone).

**There is no level or tier field, deliberately.** Every check runs on every repo and every result is shown. `applies_when` is the only thing that suppresses a check, and it fires on *fact* (no UI to test, no autonomous loop to sandbox) — never on a judgement about how ambitious the repo should be.
...
### 3.5 — Project memory
`group: guidelines` · `tip: 3.5` · `layer: A`
`applies_when:` always
...
### 3.6 — Skills
`group: guidelines` · `tip: 3.6` · `layer: A`
`applies_when:` always
...
### 3.7 — MCP servers
`group: guidelines` · `tip: 3.7` · `layer: A`
`applies_when:` always
...
`group: guidelines` · `tip: 3.8` · `layer: A`
...
### 6.2 / 6.4 — Subagents
`group: guidelines` · `tip: 6.2` (roles → `6.4`) · `layer: A`
`applies_when:` always
...
`group: guidelines` · `tip: 6.5` · `layer: A`
...
`group: autotests` · `tip: 4.1` · `layer: B`
...
### 4.2 — Unit test suite
`group: autotests` · `tip: 4.2` · `layer: B`
`applies_when:` repo contains source code (not docs-only)
...
`group: autotests` · `tip: 4.3` · `layer: B`
...
`group: autotests` · `tip: 4.4` · `layer: B`
...
`group: autotests` · `tip: 4.7` · `layer: A`
*(Worth stating because it reads as a team practice and isn't: 4.7 is about **context**, not headcount — the session that wrote the code shouldn't grade it. That bites hardest solo, where nobody else ever looks.)*
`applies_when:` always
...
### 3.2 — Secrets hygiene
`group: guardrails` · `tip: 3.2` · `layer: A`
...
`group: guardrails` · `tip: 5.1` · `layer: A`
...
### 5.2 — `gh` / PR flow
`group: guardrails` · `tip: 5.2` · `layer: A`
...
### 5.4 — Test hook
`group: guardrails` · `tip: 5.4` · `layer: A`
...
### 5.5 — CI
`group: guardrails` · `tip: 5.5` · `layer: A`
`applies_when:` the repo has a remote
...
### 8.1 — Sandboxing
`group: guardrails` · `tip: 8.1` · `layer: A`
...
### 8.2 — Approval gate
`group: guardrails` · `tip: 8.2` · `layer: A`
...
### 8.3 — Strike cap
`group: guardrails` · `tip: 8.3` · `layer: A`
`applies_when:` autonomous-loop config present
...
Rendered only under `--full`. Uncle Bob's six constraints from the tweet in the guide's Big Idea, mapped to what the guide teaches.

*(`--full` no longer controls the tier split — every check runs and is shown on every run. This section is the extra: the gauntlet framing, and the honest note on what the guide doesn't cover.)*
...
| Test coverage | `[Tip 4.10]` | a **diff**-coverage gate on changed lines, wired into CI — not a global percentage |
| Mutation testing | `[Tip 4.11]` | Stryker · mutmut · cargo-mutants · PIT, scoped to changed files |
