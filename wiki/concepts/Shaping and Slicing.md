...
- **2.1 Investigate before you edit — a wrong mental model wrecks the diff.** Explore and explain first, change nothing, *then* implement.
- **2.2 Plan the uncertain; skip the plan for the trivial.** Plan multi-file or unfamiliar work; do one-sentence diffs directly.
- **2.3 Force an approval checkpoint and see what it will touch.** Numbered plan, risk per step, exact files to create/modify/delete, then STOP for approval — nothing irreversible runs unseen.
- **2.4 Ask for options, not the first idea.** "Give me 2–3 approaches with a one-line trade-off each; I'll choose."
- **2.5 Slice vertically — thin end-to-end increments, not horizontal layers.** UI → API → DB → test, ship green before the next slice. See [[Vertical Slicing]].
- **2.6 Turn big features into a spec, then a fresh session.** Interview me, write SPEC.md → new session: "Implement SPEC.md." Shape behavior as a Story, Given/When/Then scenarios, and Non-goals.
- **2.7 Plan with the smart model, build with the cheap one.** `/model opusplan` — Opus reasons through the plan, Sonnet writes the code.
- **2.8 Have one agent draft the spec and a different one critique it — before any code.** The critic's different blind spots are the point. See [[Multi-Model Playbook]].
