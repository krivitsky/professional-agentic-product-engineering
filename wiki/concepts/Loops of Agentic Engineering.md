...
- **Coding loop** (red → green → refactor) — "closed by Tier 4's tests; Tier 5 hardens that check so it runs every time." (see [[Loop Until Done]])
- **Feature loop** (specify → refine → verify) — "runs from the spec you shape in Tier 2 to that same Tier 4 check, which is why Tier 4 belongs to both." (see [[Shaping and Slicing]])
- **Impact loop** (opportunities → hypothesis → impact) — "Tiers 6–8 scale out to it, once one agent or one machine isn't enough." (see [[Agent Execution Layer]])
